...
function AdminPage() {
  ...
  const fetchModelAccess = useServerFn(getModelAccess);
  ...
  useEffect(() => {
    Promise.all([fetchRole({}), fetchModelAccess({})])
      .then(([role, modelAccess]) => {
        setIsAdmin(role.isAdmin);
        setCanManageModels(modelAccess.allowed);
      })
      .catch(() => {
        setIsAdmin(false);
        setCanManageModels(false);
      })
